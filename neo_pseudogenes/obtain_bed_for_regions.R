...
gff_t1 <- read_tsv(str_glue("results/i-ADHoRe-run2/processing/{multiplicon}/tmp_gff_{haplotype_1}"), 
                   col_names = 
                     c("chrom", "source", "type", "start", "end", "x",
                                 "or", "y", "gene"),
                   show_col_types = FALSE)
reg_t1 <- read_tsv(str_glue("results/i-ADHoRe-run2/processing/{multiplicon}/region_{haplotype_1}_{chrom_1}.txt"), 
                        col_names = FALSE, show_col_types = FALSE)
# read in data of second region
gff_t2 <- read_tsv(str_glue("results/i-ADHoRe-run2/processing/{multiplicon}/tmp_gff_{haplotype_2}"), 
                   col_names = 
                     c("chrom", "source", "type", "start", "end", "x",
                       "or", "y", "gene"),
                   show_col_types = FALSE)
reg_t2 <- read_tsv(str_glue("results/i-ADHoRe-run2/processing/{multiplicon}/region_{haplotype_2}_{chrom_2}.txt"), 
                   col_names = FALSE, show_col_types = FALSE)
...
write_tsv(bed_r1, str_glue("results/i-ADHoRe-run2/processing/{multiplicon}/region_{haplotype_1}_{chrom_1}.bed"),
          col_names = FALSE)
...
write_tsv(bed_r2, str_glue("results/i-ADHoRe-run2/processing/{multiplicon}/region_{haplotype_2}_{chrom_2}.bed"),
          col_names = FALSE)
...
coords_r1 %>% mutate(start = start - bed_r1$start - 1,
  ...
  write_tsv(str_glue("results/i-ADHoRe-run2/processing/{multiplicon}/genes_{haplotype_1}_{chrom_1}.bed"),
            col_names = FALSE)
coords_r2 %>% mutate(start = start - bed_r2$start - 1,
                     end = end - bed_r2$start,
                     name = str_glue("{haplotype_2}_{chrom_2}")) %>%
  select(name, start, end, X1) %>%
  write_tsv(str_glue("results/i-ADHoRe-run2/processing/{multiplicon}/genes_{haplotype_2}_{chrom_2}.bed"),
            col_names = FALSE)

# save region start coordinates
write_tsv(bed_r1 %>% select(start), 
          str_glue("results/i-ADHoRe-run2/processing/{multiplicon}/region_{haplotype_1}_{chrom_1}_start.txt"),
          col_names = FALSE)
write_tsv(bed_r2 %>% select(start), 
          str_glue("results/i-ADHoRe-run2/processing/{multiplicon}/region_{haplotype_2}_{chrom_2}_start.txt"),
          col_names = FALSE)
